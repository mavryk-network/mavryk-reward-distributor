...
The configuration parameters for all plugins are located in the validators .yaml config file. Please take a look at [the example config](https://github.com/mavryk-network/mavryk-reward-distributor/blob/master/examples/mv1BooTWe9vnuvN1o756pE6yC8jNcsVDCp9N.yaml).
...
* telegram_text: This text will be sent to all chat ids list in 'payouts_chat_ids'. The text will be passed through a filter to replace the following placeholders: `%CYCLE%`, `%NDELEGATORS%`, `%TREWARDS%`, `%VALIDATOR%`. Basic HTML formatting is supported; see Telegram Docs for supported HTML tags. Emojis and other unicode characters are supported.
...
plugins:
  telegram:
    ...
    telegram_text: >
      &#x2728; Reward for cycle %CYCLE% <b>complete</b>! Validator: %VALIDATOR%. We had %NDELEGATORS% <i>delegators</i> in the cycle and paid out %TREWARDS% mav in rewards!
```
...
**NOTE**: `tweet_text` will be passed through a filter to replace the following placeholders: `%CYCLE%`, `%NDELEGATORS%`, `%TREWARDS%`, `%VALIDATOR%`
...
plugins:
  twitter:
    ...
    tweet_text: >
      Reward for cycle %CYCLE% complete! Validator: %VALIDATOR%. We had %NDELEGATORS% delegators in the cycle and paid out %TREWARDS% mav in rewards. #rewards #mavryk
```

The above example configuration will produce a tweet that looks like this:

```
Reward for cycle 290 complete! Validator: mv1BooTWe9vnuvN1o756pE6yC8jNcsVDCp9N. We had 133 delegators
...
* discord_text: This text will be sent to the channel configured for your bot. The text will be passed through a filter to replace the following placeholders: `%CYCLE%`, `%NDELEGATORS%`, `%TREWARDS%`, `%VALIDATOR%`. Markdown formatting is supported; Emojis and other unicode characters are supported.
...
plugins:
  ...
  discord:
    ...
    send_admin: False
    discord_text: >
      Rewards for cycle %CYCLE% are completed. Validator: %VALIDATOR%.
